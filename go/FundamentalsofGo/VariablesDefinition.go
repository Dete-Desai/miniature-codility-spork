...
func main() {
	...
	fmt.Printf("%s %s is %d years with a height of %f.", firstName, secondName, age, height)
	fmt.Printf("His religion is %s with a %s degree from Havard.", religion, occupation)
	fmt.Printf("%s likes jordan shoes with a shoe size of %f because of his weight of %d\n", firstName, shoeSize, weight)
}
